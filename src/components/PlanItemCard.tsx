...
export default function PlanItemCard({ item, onEdit, onDelete }: PlanItemCardProps) {
    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative group">
            ...
            <div className="absolute right-12 top-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                <button
                    onClick={(e) => {
                        e.stopPropagation();
                        onEdit();
            ...
            <button
                ...
                className="absolute right-3 top-3 p-1.5 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-lg transition-colors z-10"
            >
            ...
            <div
                className={`p-4 cursor-pointer transition-colors hover:bg-gray-50/50 ${isExpanded ? 'border-b border-gray-100' : ''}`}
                onClick={() => setIsExpanded(!isExpanded)}
            >
                <h3 className="text-lg font-bold text-gray-900 pr-24">{item.name}</h3>
            </div>
